  ...
  return (
    <section id="home" className="relative bg-navy text-white overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-28 sm:py-36 text-center">
        <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur text-sm font-medium px-4 py-1.5 rounded-full mb-8 border border-white/10">
        ...
        <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
          ...
          <a
            href="#how-it-works"
            className="border border-white/20 text-white font-semibold px-8 py-3.5 rounded-lg text-lg hover:bg-white/5 transition-all w-full sm:w-auto"
          >
            Watch Demo
          </a>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-14 grid grid-cols-1 md:grid-cols-3 gap-6">
          {stats.map((s) => (
            ...
            >
              <div className={`text-4xl sm:text-[2.75rem] font-bold whitespace-nowrap ${s.color}`}>
                {s.value}
              </div>
              <p className="mt-4 text-slate-700 font-medium">{s.label}</p>
              <p className="mt-2 text-xs text-slate-400 uppercase tracking-wide">{s.source}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <section id="about" className="bg-gray-bg py-20 sm:py-28">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-14 grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl p-8 border border-gray-border shadow-sm">
            <div className="flex items-center gap-3 mb-6">
              <span className="w-10 h-10 bg-teal/10 rounded-lg flex items-center justify-center text-xl">
                🔒
              </span>
        ...
        <div className="mt-6 bg-navy rounded-2xl p-8 text-white shadow-lg">
          <h3 className="text-xl font-bold text-center mb-6">
            Connected Intelligence
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-center">
            {connections.map((c) => (
              <div key={c.from} className="flex flex-col items-center gap-2">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((f) => (
            <div
              key={f.title}
              className="group bg-gray-bg rounded-2xl p-6 border border-gray-border hover:border-teal/30 hover:shadow-md transition-all"
            >
        ...
        <div className="mt-14 grid grid-cols-1 md:grid-cols-3 gap-8">
          {steps.map((s, i) => (
            <div key={s.num} className="text-center relative">
              {i < steps.length - 1 && (
                <div className="hidden md:block absolute top-8 left-[60%] w-[80%] h-px bg-gray-border" />
              )}
              <div className="relative mx-auto w-16 h-16 bg-teal rounded-2xl flex items-center justify-center text-white text-2xl font-bold shadow-lg shadow-teal/20">
                {s.num}
              </div>
              <span className="mt-5 block text-3xl">{s.icon}</span>
        ...
        <div className="mt-14 max-w-4xl mx-auto">
          <div className="space-y-3">
            {apps.map((app) => (
              <div
                key={app.name}
                className="flex items-center justify-between bg-red-50/60 rounded-xl px-6 py-4 border border-red-100"
              >
          ...
          <div className="mt-4 text-center text-slate-400 font-bold text-xl">
            = $727–$3,459/yr
          </div>

          <div className="mt-6 bg-gradient-to-br from-navy to-navy-dark rounded-2xl p-8 text-center text-white shadow-xl">
            <div className="text-xs font-bold text-teal uppercase tracking-widest">
              VS
            </div>
            <div className="mt-3 text-3xl sm:text-4xl font-bold">
              FamilyOS{" "}
              <span className="text-teal">$149–$399/yr</span>
            </div>
            <div className="mt-2 text-lg text-slate-300">Save 50–90%</div>
            <a
              href="#pricing"
              className="mt-6 inline-block bg-teal text-white font-semibold px-8 py-3 rounded-lg hover:bg-teal-hover transition-all shadow-lg shadow-teal/20"
            >
              See Pricing
            </a>
          </div>
  ...
  const tiers = [
    {
      ...
      popular: false,
      features: ["Document vault", "Basic portfolio tracking", "AI auto-filing", "5 shared members", "10 GB storage"],
    },
    {
      name: "Premium",
      price: 249,
      ...
      popular: true,
      features: ["Full AI insights suite", "Unlimited sharing", "Cross-pillar intelligence", "Priority support", "50 GB storage"],
    },
    {
      name: "Family Pro",
      price: 399,
      ...
      popular: false,
      features: ["White-glove onboarding", "Advanced estate tools", "API access", "Dedicated success manager", "Unlimited storage"],
    },
  ];
    ...
    <section id="pricing" className="bg-gray-bg py-20 sm:py-28">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto">
          ...
          <div className="mt-8 inline-flex items-center bg-white rounded-full p-1 border border-gray-border shadow-sm">
            <button
              onClick={() => setAnnual(false)}
              className={`px-5 py-2 rounded-full text-sm font-medium transition-colors ${
                !annual ? "bg-navy text-white shadow-sm" : "text-slate-500 hover:text-navy"
              }`}
            >
              Monthly
            </button>
            ...
            >
              Annual <span className="text-teal text-xs font-bold">Save 20%</span>
            </button>
          </div>
        </div>
          ...
          {tiers.map((tier) => (
            <div
              ...
              className={`relative bg-white rounded-2xl p-8 flex flex-col transition-shadow ${
                tier.popular
                  ? "border-2 border-teal shadow-xl shadow-teal/10 scale-[1.03]"
                  : "border border-gray-border shadow-sm hover:shadow-md"
              }`}
            >
              {tier.popular && (
              ...
              <a
                ...
                className={`mt-8 block text-center font-semibold py-3 rounded-lg transition-all ${
                  tier.popular
                    ? "bg-teal text-white hover:bg-teal-hover shadow-lg shadow-teal/20"
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-12 space-y-3">
          {faqs.map((faq, i) => (
            <div
              key={i}
              className="border border-gray-border rounded-xl overflow-hidden bg-gray-bg/50"
            >
              <button
              ...
              >
                ...
                <svg
                  className={`w-5 h-5 text-slate-400 shrink-0 ml-4 transition-transform ${openIndex === i ? "rotate-180" : ""}`}
